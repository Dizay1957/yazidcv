...
public class CvProcessingService {
 ...
 private final CvDocumentRepository documents; private final CandidateRepository candidates; private final CvProfileExtractor extractor; private final AutoDetectParser parser=new AutoDetectParser();
 public CvProcessingService(CvDocumentRepository documents,CandidateRepository candidates,CvProfileExtractor extractor){this.documents=documents;this.candidates=candidates;this.extractor=extractor;}
 ...
 private Candidate resolveCandidate(CvDocument doc,String text){var p=extractor.parse(text,doc.getOriginalFilename());Candidate c;if(doc.getCandidateId()!=null)c=candidates.findByIdAndOrganizationId(doc.getCandidateId(),doc.getOrganizationId()).orElseThrow();else if(p.email()!=null&&candidates.findFirstByOrganizationIdAndEmailIgnoreCase(doc.getOrganizationId(),p.email()).isPresent())c=candidates.findFirstByOrganizationIdAndEmailIgnoreCase(doc.getOrganizationId(),p.email()).orElseThrow();else c=new Candidate(doc.getOrganizationId(),p.firstName(),p.lastName());c.update(p.firstName(),p.lastName(),p.email(),p.phone(),p.currentTitle(),p.currentCompany(),p.city(),p.country(),p.yearsExperience(),"NEW",p.summary(),p.skills());c.applyCvInsight(p.linkedinUrl(),p.githubUrl(),p.languages(),p.education(),p.experience(),p.projects(),p.confidence());return candidates.save(c);}
